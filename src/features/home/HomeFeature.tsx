import { HomeStats } from "./types";
import { PermissionGate } from "../shared";
import { getHomeStats } from "./api/getHomeStats";
import {
  HomeAnalytics,
  EntryChart,
  OverrideWordOfTheDay,
  PendingEntries,
  PopularWords,
  RecentUsers,
  RegistrationByPlatform,
  ScheduledWordOfTheDay,
  SetWordOfTheDay,
  TopUsers,
  WordOfTheDay,
} from "./components";

const HomeFeature = async ({ query }: { query: { page: string } }) => {
  const { data }: { data: HomeStats | undefined } =
    await getHomeStats();

  const { page = "home" } = query;

  if (page == "newWord") return <SetWordOfTheDay />;
  if (page == "overrideWord") return <OverrideWordOfTheDay />;

  return (
    <div className="min-h-screen p-4 md:p-6 lg:p-8">
      <div className="w-full mx-auto pb-20 overflow-hidden">
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
          <div className="flex-1 flex flex-col gap-6 lg:gap-8 min-w-0">
            <HomeAnalytics stats={data?.stats} />

            <EntryChart data={data?.wordGrowthStats} />

            <div className="flex max-md:flex-col w-full items-start gap-5 min-w-0">
              <WordOfTheDay data={data?.wordOfTheDay} />
              <ScheduledWordOfTheDay data={data?.scheduledWords} />
            </div>

            <PermissionGate permission="view_user">
              <div className="flex max-md:flex-col w-full items-start gap-5 min-w-0">
                <RecentUsers data={data?.recentUsers} />

                <TopUsers data={data?.topUsers} />
              </div>
            </PermissionGate>

            {data?.recentWords && data.recentWords.length > 0 && (
              <PendingEntries data={data.recentWords} />
            )}

            <div className="flex max-md:flex-col w-full items-start gap-5 min-w-0">
              {data?.trendingWords && data.trendingWords.length > 0 && (
                <PopularWords data={data.trendingWords} />
              )}

              {data?.registrationByPlatform &&
                data.registrationByPlatform.length > 0 && (
                  <RegistrationByPlatform data={data.registrationByPlatform} />
                )}
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};
export default HomeFeature;
