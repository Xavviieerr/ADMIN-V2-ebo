...
import { useOverrideWordOfDayMutation } from "@/slice/requestSlice";
...
const OverrideWordOfTheDay = () => {
  ...
  const [overrideWordOfDay, { isLoading }] = useOverrideWordOfDayMutation();

  const handleOverride = async () => {
    if (!selected || isLoading) return;

    try {
      await overrideWordOfDay({
        wordId: selected.id,
      }).unwrap();

      toast.success(t("common.success"), {
        description: t("home.overrideSuccess"),
      });
      clearSearch();
    } catch {
      toast.error(t("common.error"), {
        description: t("home.overrideFailed"),
      });
    }
  };
